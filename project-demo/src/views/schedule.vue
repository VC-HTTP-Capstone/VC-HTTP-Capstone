    ...
    <table class="table table-striped faded">
      ...
      <transition-group name="tbody" tag="tbody">
        <tr v-for="(todoItem, index) in propsdata" :key="todoItem">
            ...
            <td>{{ todoItem }}</td>
            <td>
              <span class="removeBtn" type="button" @click="moveToWorking(todoItem, index)">
              <i class="fa fa-play" style="color: green" aria-hidden="true"></i>
              </span>
            </td>
            <td>
...
export default {
  ...
  methods: {
    ...
    moveToWorking(todoItem, index) {
      this.$emit('moveToWorking', todoItem, index);
    }
  }
}
</script>
